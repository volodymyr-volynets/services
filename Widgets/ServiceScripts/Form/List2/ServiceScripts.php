...
namespace Numbers\Services\Widgets\ServiceScripts\Form\List2;
class ServiceScripts extends \Object\Form\Wrapper\List2 {
	public $form_link = 'wg_service_scripts';
	...
	public function renderSSValue(& $form, & $options, & $value, & $neighbouring_values) {
		// legacy import data
		if (!empty($neighbouring_values['wg_ss_legacy_answers'])) {
			return nl2br($neighbouring_values['wg_ss_legacy_answers']);
		} else { // we need to render
			return \Numbers\Services\Services\Helper\ServiceScript\Helper::renderAsText(
				$neighbouring_values['wg_ss_service_script_id'],
				$neighbouring_values['wg_ss_channel_id'],
				$neighbouring_values['wg_ss_region_id'],
				$neighbouring_values['wg_ss_language_code'],
				json_decode($neighbouring_values['wg_ss_answers'], true),
				$neighbouring_values['wg_ss_total_amount'] . ''
			);
		}
	}
	...
	public function listQuery(& $form) {
		...
		$form->query = \Factory::model($parent_model->service_scripts_model)->queryBuilder()->select();
		$form->processReportQueryFilter($form->query);
		// additional filter
		if (!empty($parent_model->service_scripts['map'])) {
			foreach ($parent_model->service_scripts['map'] as $k => $v) {
