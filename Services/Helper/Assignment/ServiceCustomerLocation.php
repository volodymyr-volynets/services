...
class ServiceCustomerLocation
{
    ...
    public function __construct(\Object\Form\Wrapper\Base & $parent)
    {
        ...
        $parent->containers['service_customer_location_assignment_container'] = [
            ...
            'details_pk' => ['ss_servcustloc_organization_id', 'ss_servcustloc_service_id', 'ss_servcustloc_customer_id'],
            'order' => 35005,
        ...
        $parent->elements['service_customer_location_assignment_container'] = [
            'row1' => [
                'ss_servcustloc_customer_id' => ['order' => 1, 'row_order' => 100, 'label_name' => 'Customer', 'domain' => 'customer_id', 'null' => true, 'required' => true, 'percent' => 50, 'method' => 'select', 'tree' => true, 'searchable' => true, 'options_model' => '\Numbers\Users\Organizations\Model\Customers::optionsJson', 'options_options' => ['show_all' => true], 'onchange' => 'this.form.submit();', 'json_contains' => ['organization_id' => 'ss_servcustloc_organization_id', 'customer_id' => 'ss_servcustloc_customer_id']],
    ...
    public function validate(& $form)
    {
        ...
        foreach ($form->values['\Numbers\Services\Services\Model\Assignment\ServiceCustomerLocation\Locations'] as $k => $v) {
            if (empty($v['ss_servcustloc_service_id'])) {
                continue;
            }
            ...
            if ($ids['queue_method_id'] == 20) {
                if (\Math::compare($v['ss_servcustloc_priority_percent'], '0', 2) == 0) {
                    $form->error(DANGER, Messages::REQUIRED_FIELD, "\Numbers\Services\Services\Model\Assignment\ServiceCustomerLocation\Locations[{$k}][ss_servcustloc_priority_percent]");
                }
            }
        }
    }
