...
class PreloadIDs extends \Object\DataSource {
	public $db_link;
	public $db_link_flag;
	...
	public function query($parameters, $options = []) {
		...
		if (!empty($parameters['channel_code'])) {
			$this->query->join('INNER', new \Numbers\Services\Services\Model\Service\Channels(), 'b', 'ON', [
		...
		if (!empty($parameters['location_id'])) {
			$this->query->join('INNER', new \Numbers\Users\Organizations\Model\Locations(), 'c', 'ON', [
		...
		$this->query->join('INNER', new \Numbers\Users\Organizations\Model\Queue\Types(), 'd', 'ON', [
