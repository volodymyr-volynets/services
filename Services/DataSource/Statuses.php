...
use Helper\Tree;
use Numbers\Services\Services\Model\Service\Status\Groups;
use Numbers\Services\Services\Model\Service\Types;
use Object\DataSource;
use Object\Table\Options;

class Statuses extends DataSource
{
    public $db_link;
    public $db_link_flag;
    ...
    public function query($parameters, $options = [])
    {
        ...
        $this->query->join('INNER', new Groups(), 'c', 'ON', [
    ...
    public function optionsJson($options = [])
    {
        ...
        foreach ($data as $k => $v) {
            foreach ($v as $k2 => $v2) {
                foreach ($v2 as $k3 => $v3) {
                    foreach ($v3 as $k4 => $v4) {
                        // service type group
                        $service_type_group_code = Options::optionJsonFormatKey([
                        ...
                        // service type code
                        $service_type_code = Options::optionJsonFormatKey([
                            'service_type_group_code' => $k,
                        ...
                        // status group code
                        $status_group_code = Options::optionJsonFormatKey([
                            'service_type_group_code' => $k,
                            'service_type_code' => $k2,
                        ...
                        // status group code
                        $status_code = Options::optionJsonFormatKey([
                            'service_type_group_code' => $k,
                            'service_type_code' => $k2,
                            'status_group_code' => $k3,
                            'status_code' => $k4
        ...
        if (!empty($result)) {
            ...
            $result = [];
            Tree::convertTreeToOptionsMulti($converted, 0, ['name_field' => 'name', 'i18n' => 'skip_sorting'], $result);
        }
        return $result;
    }
