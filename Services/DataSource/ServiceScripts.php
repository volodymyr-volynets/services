...
use Numbers\Services\Services\Model\ServiceScript\Question\Answers;
use Numbers\Services\Services\Model\ServiceScript\Question\Channels;
use Numbers\Services\Services\Model\ServiceScript\Question\Description;
use Numbers\Services\Services\Model\ServiceScript\Question\Regions;
use Object\DataSource;

class ServiceScripts extends DataSource
{
    public $db_link;
    public $db_link_flag;
    public $pk = ['id'];
    ...
    public function query($parameters, $options = [])
    {
        ...
        // join
        $this->query->join('LEFT', new Description(), 'b', 'ON', [
        ...
        $this->query->join('LEFT', function (& $query) use ($parameters) {
            $query = Answers::queryBuilderStatic(['alias' => 'inner_c', 'skip_acl' => true])->select();
            $query->columns([
        ...
        $this->query->join('LEFT', new Models(), 'd', 'ON', [
        ...
        $this->query->where('AND', function (& $query) use ($parameters) {
            ...
            if (!empty($parameters['region_id'])) {
                $query->where('OR', function (& $query) use ($parameters) {
                    $query = Regions::queryBuilderStatic(['alias' => 'inner_a'])->select();
                    $query->columns(1);
        ...
        $this->query->where('AND', function (& $query) use ($parameters) {
            ...
            if (!empty($parameters['channel_id'])) {
                $query->where('OR', function (& $query) use ($parameters) {
                    $query = Channels::queryBuilderStatic(['alias' => 'inner_b'])->select();
                    $query->columns(1);
