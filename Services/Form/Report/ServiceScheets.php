...
use Numbers\Services\Services\DataSource\ServiceScripts;
use Numbers\Services\Services\Model\Services;
use Object\Form\Wrapper\Report;

class ServiceScheets extends Report
{
    ...
    public function buildReport(& $form)
    {
        ...
        } elseif ($form->values['type_code'] == 'CUSTOMER') {
            $report->addHeader(DEF, 'row1', [
        ...
        if (!empty($service['ss_service_service_script_id'])) {
            $service_script = ServiceScripts::getStatic([
                'where' => [
