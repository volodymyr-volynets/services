...
class ServiceScripts extends Base
{
        ...
        foreach ($form->values['\Numbers\Services\Services\Model\ServiceScript\Questions'] as $k => $v) {
            ...
            if (in_array($v['ss_servquestion_type_code'], ['select', 'multiselect'])) {
                if (empty($v['ss_servquestion_model_id']) && empty($v['\Numbers\Services\Services\Model\ServiceScript\Question\Answers'])) {
                    $form->error(DANGER, Messages::REQUIRED_FIELD, "\Numbers\Services\Services\Model\ServiceScript\Questions[{$k}][ss_servquestion_model_id]");
                    $form->error(DANGER, Messages::REQUIRED_FIELD, "\Numbers\Services\Services\Model\ServiceScript\Questions[{$k}][\Numbers\Services\Services\Model\ServiceScript\Question\Answers][1][ss_servquesanswer_name]");
                }
            }
